...
if ($isAuthenticated) {
  $stmt = $pdo->prepare("SELECT id, name, birth_date, birth_time, birth_place, naksatra, moon
    FROM devs
    WHERE id = ?");
  $stmt->execute([$_GET['checkNaksatra']]);
  $person = $stmt->fetch(PDO::FETCH_ASSOC);

  // get naksatra
  $client = new \GuzzleHttp\Client();
  $response = $client->request('GET', 'https://www.prokerala.com/astrology/search.php?index=0&q=' . $person['birth_place']);
  ...
  */
  ...
  echo json_encode(['naksatra' => $matches[1]]);
